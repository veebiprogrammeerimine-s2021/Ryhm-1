<?php
	...
	function save_news($news_title, $news, $expire_date, $file_name){
		...
		if(!empty($file_name)){
			$stmt = $conn->prepare("INSERT INTO vpr_newsphotos (userid, filename) VALUES(?, ?)");
			echo $conn->error;
		...
		//nüüd uudis ise
		$stmt = $conn->prepare("INSERT INTO vpr_news (userid, title, content, photoid, expire) VALUES (?, ?, ?, ?, ?)");
		echo $conn->error;
	...
	function latest_news($limit){
		...
		$conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$conn->set_charset("utf8");
		$stmt = $conn->prepare("SELECT title, content, vpr_news.added, filename FROM vpr_news LEFT JOIN vpr_newsphotos on vpr_newsphotos.id = vpr_news.photoid WHERE vpr_news.expire >= ? AND vpr_news.deleted IS NULL GROUP BY vpr_news.id ORDER By vpr_news.id DESC LIMIT ?");
		echo $conn->error;
